...
test('should not report errors', () => {
    const autocompleteResult = parseClickHouseQueryWithCursor(
        'GRANT ON CLUSTER cluster_name SELECT(column1, column2) ON db.table TO test_user1, user2 WITH GRANT OPTION WITH REPLACE OPTION;|',
    );
...
test('should not report errors without optional parameters', () => {
    const autocompleteResult = parseClickHouseQueryWithCursor(
        'GRANT SELECT ON table TO test_user1;|',
    );
    expect(autocompleteResult.errors).toHaveLength(0);
});
...
test('should suggest keywords after table identifier', () => {
    const autocompleteResult = parseClickHouseQueryWithCursor('GRANT SELECT ON table |');
    expect(autocompleteResult.suggestKeywords).toEqual([
        {
            value: 'TO',
        },
...
test('should suggest keywords after user identifier', () => {
    const autocompleteResult = parseClickHouseQueryWithCursor('GRANT SELECT ON table TO user |');
    expect(autocompleteResult.suggestKeywords).toEqual([
        {
            value: 'WITH',
        },
...
test('should suggest keywords after WITH', () => {
    const autocompleteResult = parseClickHouseQueryWithCursor(
        'GRANT SELECT ON table TO user WITH |',
    );
    expect(autocompleteResult.suggestKeywords).toEqual([
        {
            value: 'REPLACE',
...
test('should suggest keywords after WITH GRANT', () => {
    const autocompleteResult = parseClickHouseQueryWithCursor(
        'GRANT SELECT ON table TO user WITH GRANT |',
    );
    expect(autocompleteResult.suggestKeywords).toEqual([
        {
            value: 'OPTION',
...
test('should suggest keywords after WITH REPLACE', () => {
    const autocompleteResult = parseClickHouseQueryWithCursor(
        'GRANT SELECT ON table TO user WITH REPLACE |',
    );
    expect(autocompleteResult.suggestKeywords).toEqual([
        {
            value: 'OPTION',
