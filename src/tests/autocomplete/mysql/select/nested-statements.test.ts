...
test('should suggest table name for nested SELECT column', () => {
    const autocompleteResult = parseMySqlQueryWithCursor('SELECT * FROM (SELECT | FROM test_table');
...
test('should suggest double nested SELECT', () => {
    const autocompleteResult = parseMySqlQueryWithCursor('SELECT * FROM (SELECT * FROM (|');
    const selectKeyword: KeywordSuggestion = {value: 'SELECT'};
...
test('should report errors on missing alias', () => {
    const autocompleteResult = parseMySqlQueryWithoutCursor(
