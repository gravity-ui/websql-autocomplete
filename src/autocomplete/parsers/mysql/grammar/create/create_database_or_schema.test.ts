...
test('should suggest IF NOT EXISTS for database creation', () => {
    const parseResult = parseMySqlQuery('CREATE DATABASE ', '');
...
test('should suggest IF NOT EXISTS for schema creation', () => {
    const parseResult = parseMySqlQuery('CREATE SCHEMA ', '');
...
test('should not report errors on full statement', () => {
    const parseResult = parseMySqlQueryWithoutCursor('CREATE DATABASE test_database;');
    expect(parseResult.errors).toBeUndefined();
});

test('should not report errors on full statement and fill locations', () => {
    const parseResult = parseMySqlQueryWithoutCursor('CREATE SCHEMA test_schema;');
