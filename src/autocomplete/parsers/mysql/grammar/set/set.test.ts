...
test('should not report errors when setting string with double quotes', () => {
    const parseResult = parseMySqlQuery('SET test_table.test_column="test";', '');
...
test('should not report errors when setting boolean', () => {
    const parseResult = parseMySqlQuery('SET test_table.test_column=true;', '');

    expect(parseResult.errors).toBeUndefined();
});

test('should not report errors when setting number', () => {
    const parseResult = parseMySqlQuery('SET test_table.test_column=900000;', '');

    expect(parseResult.errors).toBeUndefined();
});

test('should properly fill locations', () => {
    const parseResult = parseMySqlQueryWithoutCursor('SET test_table.test_column="test";');
