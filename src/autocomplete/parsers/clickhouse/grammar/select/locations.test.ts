...
import {TestCase} from '../../../../test/testing';
import {clickhouseAutocompleteParser} from '../../clickhouseAutocompleteParser';

// prettier-ignore-start

beforeAll(() => {
    clickhouseAutocompleteParser.yy.parseError = function (msg: string): void {
        throw Error(msg);
    };
});

const assertAutoComplete = (testDefinition: TestCase): void => {
    const debug = false;
    expect(
        clickhouseAutocompleteParser.parseSql(
...
const assertLocations = function (options: TestCase): void {
    assertAutoComplete({
        ...
        assertLocations({
            ...
            expectedLocations: [
                ...
                },
                {
                    ...
                    expression: {
                        types: ['COLREF'],
                        columnReference: [{name: 'boo'}, {name: 'a'}],
                    },
                },
                {
                    type: 'table',
