...
test('should suggest properly after WHERE', () => {
    const parseResult = parseMySqlQueryWithCursor('SELECT * FROM test_table WHERE |');
...
test('should suggest table name and alias for column', () => {
    const parseResult = parseMySqlQueryWithCursor('SELECT * FROM test_table t1 WHERE |');
    const columnSuggestions: ColumnSuggestion = {
...
test('should suggest table name and alias (with AS) for column', () => {
    const parseResult = parseMySqlQueryWithCursor('SELECT * FROM test_table AS t1 WHERE |');
    const columnSuggestions: ColumnSuggestion = {
        tables: [{name: 'test_table', alias: 't1'}],
    };
...
test('should not report errors', () => {
    const parseResult = parseMySqlQueryWithoutCursor('SELECT * FROM test_table WHERE id = 1;');
