...
test('should suggest table names for ON clause', () => {
    const autocompleteResult = parseTrinoQueryWithCursor(
        'SELECT * FROM catalog.schema.test_table_1 JOIN catalog.schema.test_table_2 ON |',
    );
...
test('should suggest table names and aliases for ON clause', () => {
    const autocompleteResult = parseTrinoQueryWithCursor(
        'SELECT * FROM catalog.schema.test_table_1 t1 JOIN catalog.schema.test_table_2 t2 ON |',
    );
    const columnSuggestion: ColumnSuggestion = {
        tables: [
...
test('should suggest table names and aliases for ON clause if table name not unique', () => {
    const autocompleteResult = parseTrinoQueryWithCursor(
        'SELECT * FROM catalog.schema.test_table_1 t1 JOIN catalog.schema.test_table_1 t2 ON |',
    );

    const columnSuggestion: ColumnSuggestion = {
        tables: [
            {name: 'catalog.schema.test_table_1', alias: 't1'},
            {name: 'catalog.schema.test_table_1', alias: 't2'},
        ],
    };
...
test('should suggest table names and aliases (with AS) for ON clause', () => {
    const autocompleteResult = parseTrinoQueryWithCursor(
        'SELECT * FROM catalog.schema.test_table_1 AS t1 JOIN catalog.schema.test_table_2 AS t2 ON |',
    );
    const columnSuggestion: ColumnSuggestion = {
        tables: [
            {name: 'catalog.schema.test_table_1', alias: 't1'},
...
test('should suggest table names and aliases for ON clause for second column', () => {
    const autocompleteResult = parseTrinoQueryWithCursor(
        'SELECT * FROM catalog.schema.test_table_1 AS t1 JOIN catalog.schema.test_table_2 AS t2 ON id = |',
    );
    const columnSuggestion: ColumnSuggestion = {
        tables: [
            {name: 'catalog.schema.test_table_1', alias: 't1'},
...
test('should suggest table names and aliases for ON clause for second condition', () => {
    const autocompleteResult = parseTrinoQueryWithCursor(
        'SELECT * FROM catalog.schema.test_table_1 AS t1 JOIN catalog.schema.test_table_2 AS t2 ON id = id AND |',
    );
    const columnSuggestion: ColumnSuggestion = {
        tables: [
            {name: 'catalog.schema.test_table_1', alias: 't1'},
...
test('should suggest table names and aliases for WHERE clause', () => {
    const autocompleteResult = parseTrinoQueryWithCursor(
        'SELECT * FROM catalog.schema.test_table_1 AS t1 JOIN catalog.schema.test_table_2 AS t2 ON id = id WHERE |',
    );
    const columnSuggestion: ColumnSuggestion = {
        tables: [
            {name: 'catalog.schema.test_table_1', alias: 't1'},
