...
test.skip('should suggest properly after ORDER', () => {
    const autocompleteResult = parseMySqlQueryWithCursor('SELECT * FROM test_table ORDER |');
...
test('should suggest properly after ORDER', () => {
    const autocompleteResult = parseMySqlQueryWithCursor('SELECT * FROM test_table as t ORDER |');
    const keywordsSuggestion: KeywordSuggestion[] = [{value: 'BY'}];
